    ...
    <div class="content">
        ...
        <div class="top-bar">
            <div class="item">
                <el-form :inline="true">
                    ...
                    <el-form-item>
                        <el-select v-model="statusSearch" clearable placeholder="鉴定状态">
                            <el-option :key="1" :label="'待鉴定'" :value="'0'"></el-option>
        ...
        <template>
            <el-table :data="dataList.content" border style="width: 100%" :height="autoHeight" v-loading="loading">
                ...
                <el-table-column prop="identifyResult.identifierName" label="鉴定师" min-width="120"></el-table-column>
                <el-table-column prop="identifyResult.isTrue" label="鉴定结果" min-width="120">
                    <template slot-scope="scope">
                        <template v-if="dataList.content[scope.$index].identifyResult">
                ...
                <el-table-column label="鉴定状态" min-width="120">
                    <template slot-scope="scope">
                        <template v-if="dataList.content[scope.$index].identifyRequest.identifyStatus=='0'">
                          待鉴定
                        </template>
                        <template v-if="dataList.content[scope.$index].identifyRequest.identifyStatus=='1'">
                          已完成
                        </template>
                    </template>
                </el-table-column>
                        ...
                        <el-row>
                            <el-col :span="6">
                                <el-form-item label="操作状态" prop="">
                                    <el-select v-model="detailStatus" placeholder="请选择" style="width:100%" disabled>
    ...
    methods:{
        handleCurrentChange(val) {
            ...
            if(this.statusSearch){
                _searchData+='&identifyRequest.identifyStatus='+this.statusSearch
            }
            if(this.orderTypeSearch){
                _searchData+='&orderType='+this.orderTypeSearch
            }
            this.fetchData(api.ordersList+'?identifyRequest.identifyStatus=0&sort=createTime,desc&page='+(this.currentPage-1)+'&size='+this.pageSize+_searchData,'',(data) => {
                this.dataList=data
                this.loading=false
            })
        },
        ...
        save(){
            ...
            if(this.detail.sellGoods.categoryId == '1'){
                ...
                _watchData.identifyId = this.detail.identifyRequest.pkGlobalId
                _watchData.isTrue = this.jdStatus
                this.$confirm('确定保存吗？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
                    this.subForm(api.identifyReportWatch,_watchData,(data) => {
                        this.reData()
            ...
            if(this.detail.sellGoods.categoryId == '2'){
                ...
                _bagData.identifyId = this.detail.identifyRequest.pkGlobalId
