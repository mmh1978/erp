    ...
    <div class="content">
        ...
        <el-dialog title="编辑订单" :visible.sync="modifyState" size="large" :before-close="handleClose">
            ...
            <el-tabs type="border-card" v-model="activeName" @tab-click="changeTabs">
                ...
                <el-tab-pane label="基本信息" name="first">
                    ...
                    <el-form label-width="80px">
                        ...
                        <el-form-item label="发货单号">
                            <el-input v-model="subDetail.sendNumber1">
                                <template slot="append">用户发往平台</template>
                            </el-input>
                        </el-form-item>
                        <el-form-item label="发货单号">
                            <el-input v-model="subDetail.sendNumber2">
                                <template slot="append">平台发往用户</template>
                            </el-input>
                        </el-form-item>
                        <el-form-item label="备注">
                            <el-input v-model="subDetail.memo"></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <!-- the second tab -->
                <el-tab-pane label="鉴定报告" name="second" v-if="hasIdentify">
                    <h3>未鉴定</h3>
                </el-tab-pane>
                <el-tab-pane label="鉴定报告" name="second" v-else>
...
export default {
    ...
    methods:{
        handleCurrentChange(val) {
            ...
            this.fetchData(api.identifyList+'?type=0&sort=createTime,desc&page='+(this.currentPage-1)+'&size='+this.pageSize+_searchData,'',(data) => {
                this.dataList=data
                this.loading=false
            })
        },
        ...
        changeTabs(targetName) {
            if(targetName.name=='second'){
                if(this.detail.categoryId=='1'){
                    this.fetchData(api.identifyReportWatch+'?identifyId='+this.detail.pkGlobalId,'',(data) => {
                        let _data = data
                        if(_data.length>0){
                ...
                if(this.detail.categoryId=='2'){
                    this.fetchData(api.identifyReport+'?identifyId='+this.detail.pkGlobalId,'',(data) => {
                        let _data = data
                        if(_data.length>0){
                            this.hasIdentify = false
