        ...
        <div class="top-bar">
            <div class="item">
                <el-form :inline="true">
                    ...
                    <el-form-item>
                        <el-select v-model="isAgreeSearch" clearable placeholder="请选择审核状态">
            ...
            <el-table :data="dataList.content" border style="width: 100%" :height="autoHeight" v-loading="loading">
                ...
                <el-table-column fixed="right" label="操作" width="140">
                    <template slot-scope="scope">
                        ...
                        <el-button type="text" size="small" @click.native.prevent="showCode(scope.$index, dataList.content)">二维码</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </template>
...
export default {
    ...
    methods:{
        handleCurrentChange(val) {
            ...
            this.fetchData(api.sellGoods+'?page='+(this.currentPage-1)+'&size='+this.pageSize+_searchData,'',(data) => {
                this.dataList=data
                this.loading=false
            })
        },
        ...
        showCode(index , data) {
            window.open("http://pan.baidu.com/share/qrcode?w=500&h=500&url=http%3a%2f%2fddq.chaddq.com%3a8080%2fPawnChat%2fapp%2findex.html%23%2fdetail%3fgoodsId%3d" + data[index].pkGlobalId)
        },
            ...
            this.$refs[fourthFormA].validate((valid) => {
                if (valid) {
                    ...
                    if(this.fourthFormA.pkGlobalId){
                        ...
                        this.fourthFormA.goodsId=this.modifyForm.pkGlobalId
                        console.log(this.fourthFormA)
            ...
            this.$refs[fourthFormB].validate((valid) => {
                if (valid) {
                    ...
                    if(this.fourthFormB.pkGlobalId){
                        ...
                        this.fourthFormB.goodsId=this.modifyForm.pkGlobalId
                        console.log(this.fourthFormB)
        ...
        changeTabs(targetName) {
            if(targetName.name=='second'){
                ...
                this.fetchData(api.goodsAttr+'?goodsId='+this.modifyForm.pkGlobalId+'&page=0&size=999','',(data) => {
                    console.log(data)
                    if(data.content && data.content[0].categoryId=='1'){
                        this.thirdFormA=data.content[0]
                    }else if(data.content && data.content[0].categoryId=='2'){
